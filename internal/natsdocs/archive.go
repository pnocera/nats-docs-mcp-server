...
type archiveLoader struct {
	...
	includeOrphans bool
	includeLegacy  bool
	// revision labels Metadata; callers currently supply NATSArchiveBranch.
	revision string
...
func LoadArchive(ctx context.Context, cfg ArchiveOptions, logger *slog.Logger) ([]*index.Document, Metadata, error) {
	return newArchiveLoader(cfg, logger).Load(ctx)
}

func LoadArchiveIndex(ctx context.Context, cfg ArchiveOptions, logger *slog.Logger) (*index.DocumentationIndex, Metadata, error) {
	docs, meta, err := LoadArchive(ctx, cfg, logger)
	if err != nil {
		return nil, Metadata{}, err
	}

	idx := index.NewDocumentationIndex()
	if err := idx.ImportDocuments(docs); err != nil {
		return nil, Metadata{}, err
	}
	idx.SetAliases(meta.Aliases)
	return idx, meta, nil
}

func newArchiveLoader(cfg ArchiveOptions, logger *slog.Logger) *archiveLoader {
	return &archiveLoader{
		archivePath:    cfg.ArchivePath,
		archiveBytes:   cfg.ArchiveBytes,
...
func (l *archiveLoader) Load(ctx context.Context) ([]*index.Document, Metadata, error) {
	arch, origin, err := l.openArchive(ctx)
	if err != nil {
		return nil, Metadata{}, err
	}
	...
	if !ok {
		return nil, Metadata{}, fmt.Errorf("archive is missing SUMMARY.md")
	}
	...
	if err != nil {
		return nil, Metadata{}, fmt.Errorf("failed to parse SUMMARY.md: %w", err)
	}
	...
	if entry, ok := arch.Get(".gitbook.yaml"); ok {
		...
		if err != nil {
			return nil, Metadata{}, fmt.Errorf("failed to parse .gitbook.yaml: %w", err)
		}
	}
...
func (l *archiveLoader) openArchive(ctx context.Context) (*fetcher.Archive, string, error) {
	if len(l.archiveBytes) > 0 {
		arch, err := fetcher.OpenArchiveBytes(ctx, l.archiveBytes, l.limits)
		return arch, "embedded:nats.docs-master.zip", err
	}
	arch, err := fetcher.OpenLocalArchive(ctx, l.archivePath, l.limits)
	return arch, l.archivePath, err
}
